...
class LongDocFACTScore:
    def __init__(self, device="cuda:0", metric="BARTScore"):
        ...
        if metric == "BARTScore":
            ...
            self.metric = BERTScorer(model_type="ProsusAI/finbert")
            self.metric_function = self.metric.score
        else:
    ...
    def score_src_hyp_long(self, srcs, hyps, metric="BERTScore"):
        all_scores = []
        ...
        for src, hyp in zip(srcs, hyps):
            ...
            logger.debug(f"Max scores: {doc_scores}")
